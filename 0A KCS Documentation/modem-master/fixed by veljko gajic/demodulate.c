...
size_t infilesize, i;
  ...
  FILE* inputWaveFile = fopen(argv[1], "rb");
  
  
  fseek(inputWaveFile, 0L, SEEK_END);
  infilesize = ftell(inputWaveFile);
  fseek(inputWaveFile, 0L, SEEK_SET);
  ...
  {    
      /*get next sample*/
    fread(&thisSample, 1, 1, inputWaveFile);  
      /*used for debugging*/
    totalSamples++;
